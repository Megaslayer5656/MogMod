...
{
    public class SpiritJump : ExtraJump
    {
        ...
        public override void OnStarted(Player player, ref bool playSound)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                Dust dust = Dust.NewDustDirect(player.position + new Vector2(-34f, offsetY), 102, 32, DustID.PlatinumCoin, -player.velocity.X * 0.5f, player.velocity.Y * 0.5f, 100, Color.Gray, 1.5f);
        ...
        public override void ShowVisuals(Player player)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                SpawnSpiritDust(player, spawnPos, 0.1f, i == 0 ? -0.07f : -0.13f);
            }

            for (int i = 0; i < 3; i++)
            {
                SpawnSpiritDust(player, spawnPos, 0.6f, 0.8f);
            }
        ...
        private static void SpawnSpiritDust(Player player, Vector2 spawnPos, float dustVelocityMultiplier, float playerVelocityMultiplier)
        {
    ...
    public class TritonJump : ExtraJump
    {
        ...
        public override void OnStarted(Player player, ref bool playSound)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                Dust dust = Dust.NewDustDirect(player.position + new Vector2(-34f, offsetY), 102, 32, DustID.GemDiamond, -player.velocity.X * 0.5f, player.velocity.Y * 0.5f, 100, Color.Gray, 1.5f);
                dust.velocity = dust.velocity * 0.5f - player.velocity * new Vector2(0.1f, 0.3f);
            }
