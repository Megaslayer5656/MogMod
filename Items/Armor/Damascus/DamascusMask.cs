...
namespace MogMod.Items.Armor.Damascus
{
    ...
    public class DamascusMask : ModItem, ILocalizedModType
    {
        ...
        #endregion
        #region Tooltips
        public static bool HasArmorSet(Player player) => player.armor[0].type == ItemType<DamascusMask>() && player.armor[1].type == ItemType<DamascusMail>() && player.armor[2].type == ItemType<DamascusGreaves>();
            ...
            if (HasArmorSet(Main.LocalPlayer))
            {
                ...
                if (setBonusIndex != -1)
                {
                    if (Main.keyState.PressingShift())
                    {
                        ...
                        {
                            setBonusIndex++;
                            TooltipLine briefDescription = new(item.Mod, "MogMod:SetBonus1", MiscUtils.GetTextFromModItem<DamascusMask>("SetBonusGFB").Format(AbilityBriefColor.Hex3()));
                            tooltips.Insert(setBonusIndex, briefDescription);
                        }
                        else
                        {
                            setBonusIndex++;
                            TooltipLine briefDescription = new(item.Mod, "MogMod:SetBonus1", MiscUtils.GetTextFromModItem<DamascusMask>("SetBonusNormal").Format(AbilityBriefColor.Hex3()));
                            tooltips.Insert(setBonusIndex, briefDescription);
                        }
                    }
                    else
