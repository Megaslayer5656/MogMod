...
namespace MogMod.Projectiles.BaseProjectiles
{
    public abstract class BaseFlailProjectile : ModProjectile
    {
        ...
        public virtual Color SpecialDrawColor => new(0, 0, 0);
        public virtual bool SpawnDust => false;
        ...
        public virtual void GenerateDust()
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                Dust dust = Dust.NewDustDirect(Projectile.position + Projectile.velocity * 2f, Projectile.width, Projectile.height, ExudeDustType, 0f, 0f, 100, SpecialDrawColor == new Color(0, 0, 0) ? default : SpecialDrawColor, 2f);
                dust.noGravity = true;
                dust.velocity *= 2f;
            ...
            while (counter < counterMax)
            {
                if (Main.rand.NextBool(4))
                {
                    ...
                    Dust dust = Dust.NewDustDirect(spawnPosition, Projectile.width, Projectile.height, WhipDustType, 0f, 0f, 100, SpecialDrawColor == new Color(0, 0, 0) ? default : SpecialDrawColor, 1f);
                    dust.noGravity = true;
                    dust.fadeIn = 0.5f;
                    dust.velocity += Projectile.localAI[0].ToRotationVector2();
                    //dust.noLight = true;
