...
{
    public class EchoSabreHoldout : BaseCustomUseStyleProjectile, ILocalizedModType
    {
        public override int AssignedItemID => ModContent.ItemType<EchoSabre>();
        public override LocalizedText DisplayName => MiscUtils.GetItemName<EchoSabre>();
        public override string Texture => "MogMod/Items/Weapons/Melee/EchoSabre";
        public int size = 60 + 10;
                ...
                else
                {
                    ...
                    if (CanHit)
                    {
                        for (int i = 0; i < 2; i++)
                        {
                            ...
                            dust2.color = Main.rand.NextBool() ? Color.Silver : Color.LightGreen;
                        }
                    }
                }
            }
