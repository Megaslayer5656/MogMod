...
{
    public class DagonFourHoldout : BaseHoldoutProjectile
    {
        public override LocalizedText DisplayName => MiscUtils.GetItemName<DagonFour>();
        public override string Texture => "MogMod/Items/Weapons/Magic/DagonFour";
        ...
        int MaxShots = DagonFour.MaxShots; // max amount of shots fired
            ...
            else
            {
                if (Timer >= MinCharge)
                {
                    ...
                    if (!coolingDown && NumShots == 0)
                    {
                        ...
                        {
                            Dust dust2 = Dust.NewDustPerfect(Projectile.Center + Projectile.velocity * 20f, Main.rand.NextBool(2) ? DustID.FireworksRGB : DustID.Flare, -(Projectile.velocity * Main.rand.NextFloat(-2f, 2f)).RotatedByRandom(1.4f));
                    ...
                    float shakeValue = ShootTimer * 2f;
                    ...
                    int type = ModContent.ProjectileType<VolatileDagonBolt>();
                    if (NumShots > 0) Projectile.position -= shootVelocity * 1.25f * ShootTimer;
                    if (ShootTimer >= 1f * attackSpeed && canUseMana && !coolingDown)
                    {
                        if (NumShots == 0)
                        {
                            Owner.CheckMana(Owner.HeldItem, -1, true);
                            Projectile.position -= shootVelocity * 1.25f * ShootTimer;
                        }
                        if (NumShots < MaxShots)
                        {
                            SoundEngine.PlaySound(SoundID.Item109 with { Pitch = -0.2f, PitchVariance = 0.1f }, Projectile.Center);
        ...
        public override void PreDrawBehind(ref Color lightColor)
        {
            ...
            Texture2D ghost = ModContent.Request<Texture2D>("MogMod/Assets/Ghosts/DagonFourGhost").Value;
