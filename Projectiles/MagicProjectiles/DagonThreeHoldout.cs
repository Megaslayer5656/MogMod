...
{
    public class DagonThreeHoldout : BaseHoldoutProjectile
    {
        public override LocalizedText DisplayName => MiscUtils.GetItemName<DagonThree>();
        public override string Texture => "MogMod/Items/Weapons/Magic/DagonThree";
        ...
        public override void HoldoutAI()
        {
            ...
            else
            {
                if (Timer >= MinCharge)
                {
                    ...
                    {
                        // dust effect
                        int dustNum = (int)MathHelper.Clamp(ShootTimer * 3f, 1f, 3f);
                        for (int i = 0; i <= dustNum; i++)
                        {
                            Dust dust2 = Dust.NewDustPerfect(Projectile.Center + Projectile.velocity * 20f, Main.rand.NextBool(5) ? DustID.FireworksRGB : DustID.Flare, -(Projectile.velocity * Main.rand.NextFloat(-2f, 2f)).RotatedByRandom(1.4f));
                    ...
                    int type = ModContent.ProjectileType<ExplosiveDagonBolt>();
                    if (ShootTimer >= 1f * attackSpeed && canUseMana && !coolingDown)
                    {
                        Owner.CheckMana(Owner.HeldItem, -1, true);
                        SoundEngine.PlaySound(SoundID.Item109 with { Pitch = -0.1f, PitchVariance = 0.1f }, Projectile.Center);
                    ...
                    if (SoundEngine.TryGetActiveSound(AudSlot, out var ChargeSound) && ChargeSound.IsPlaying)
                    {
                        ...
                        ChargeSound.Pitch = Utils.Remap(ShootTimer, 0, 1f, -0.6f, 0f);
                        ChargeSound.Volume = Utils.Remap(ShootTimer, 0, 1f, 0f, 0.9f) * 100;
                    }
