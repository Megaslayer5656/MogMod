                ...
                if (player.HasAmmo(heldItem) && !player.noItems && !player.CCed)
                {
                    ...
                    for (int j = 0; j < projectileCount; j++)
                    {
                        ...
                        if (ammoConsumed)
                        {
                            ...
                            Projectile.NewProjectile(source, spawnLocation, Vector2.Normalize(Projectile.velocity) * (speed), ModContent.ProjectileType<ArchbeastArrow>(), damage * 8, knockBack * 3, Projectile.owner);
                        }
                    }
                    Projectile.Kill();
                }
