...
namespace MogMod.Projectiles.RangedProjectiles
{
    public class ArchbeastBowCharge : ModProjectile, ILocalizedModType
    {
            ...
            else
            {
                ...
                if (Owner.HasAmmo(Owner.ActiveItem()))
                {
                    ...
                    if (CurrentChargingFrames >= FramesToCharge && CurrentCharge < ArchbeastParagon.MaxCharge)
                    {
                        ...
                        // cooler dust effect for max charge
                ...
                // max charge dust effect
                if (CurrentCharge == ArchbeastParagon.MaxCharge)
                {
                    Vector2 shootVelocity = Projectile.velocity.SafeNormalize(Vector2.UnitY) * -12f;
                    for (int i = 0; i <= 5; i++)
                    {
                        Dust dust = Dust.NewDustPerfect(tipPosition - Projectile.velocity * dustSpot, DustID.Flare, shootVelocity.RotatedByRandom(MathHelper.ToRadians(16f)) * Main.rand.NextFloat(0.2f, 1.2f), 0, default, Main.rand.NextFloat(1f, 2.3f));
                        dust.scale = 1.4f;
                        dust.alpha = 100;
                        dust.noGravity = true;
                    }
                ...
                // non max charge dust effect
                if (CurrentCharge >= 1 && CurrentCharge < ArchbeastParagon.MaxCharge)
                {
                    Vector2 shootVelocity = Projectile.velocity.SafeNormalize(Vector2.UnitY) * -12f;
                    for (int i = 0; i <= 5; i++)
                    {
                        Dust dust = Dust.NewDustPerfect(tipPosition - Projectile.velocity * dustSpot, DustID.Torch, shootVelocity.RotatedByRandom(MathHelper.ToRadians(8f)) * Main.rand.NextFloat(0.2f, 1.2f), 0, default, Main.rand.NextFloat(1f, 2.3f));
                        dust.scale = 0.8f;
                        dust.alpha = 100;
                        dust.noGravity = true;
                    }
                ...
                // another non max charge dust effect
                if (CurrentCharge >= 3 && CurrentCharge < ArchbeastParagon.MaxCharge)
                {
                    Vector2 shootVelocity = Projectile.velocity.SafeNormalize(Vector2.UnitY) * -12f;
                    for (int i = 0; i <= 5; i++)
                    {
                        Dust dust = Dust.NewDustPerfect(tipPosition - Projectile.velocity * dustSpot, DustID.Torch, shootVelocity.RotatedByRandom(MathHelper.ToRadians(13f)) * Main.rand.NextFloat(0.2f, 1.2f), 0, default, Main.rand.NextFloat(1f, 2.3f));
                        dust.scale = 1.1f;
                        dust.alpha = 100;
                        dust.noGravity = true;
                    }
        ...
        private void UpdateProjectileHeldVariables(Vector2 armPosition)
        {
            ...
            // where the player holds the weapon
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            for (int i = 0; i < loops; i++)
            {
                ...
                // so the arrow isnt bright, also transparent when initially drawn
