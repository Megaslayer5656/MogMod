...
namespace MogMod.Projectiles.RangedProjectiles
{
    public class ArchbeastHoldout : ModProjectile, ILocalizedModType
    {
        ...
        public override void AI()
        {
            ...
            Vector2 tipAdjustment = (Projectile.velocity * Projectile.width * 0.4f);

            float numb = 1f;
            HoldTimer += numb;
            int initialShootDelay = 200;
            ...
            if (ShootTimer == 40f || ShootTimer == 80f || ShootTimer == 120f || ShootTimer == 160f)
            {
            ...
            if (ShootTimer >= 40)
            {
                for (int i = 0; i <= 5; i++)
                {
                    Dust dust = Dust.NewDustPerfect(tipPosition - Projectile.velocity * dustSpot, DustID.Torch, shootVelocity.RotatedByRandom(MathHelper.ToRadians(5f)) * Main.rand.NextFloat(0.2f, 1.2f), 0, default, Main.rand.NextFloat(1f, 2.3f));
                    dust.scale = 0.75f;
                    dust.alpha = 100;
                    dust.noGravity = true;
                }
                if (ShootTimer >= 80)
                {
                    for (int i = 0; i <= 5; i++)
                    {
                        Dust dust = Dust.NewDustPerfect(tipPosition - Projectile.velocity * dustSpot, DustID.Torch, shootVelocity.RotatedByRandom(MathHelper.ToRadians(10f)) * Main.rand.NextFloat(0.2f, 1.2f), 0, default, Main.rand.NextFloat(1f, 2.3f));
                        dust.scale = 1f;
                        dust.alpha = 100;
                        dust.noGravity = true;
                    }
                    if (ShootTimer >= 120)
                    {
                        for (int i = 0; i <= 5; i++)
                        {
                            Dust dust = Dust.NewDustPerfect(tipPosition - Projectile.velocity * dustSpot, DustID.Flare, shootVelocity.RotatedByRandom(MathHelper.ToRadians(13f)) * Main.rand.NextFloat(0.2f, 1.2f), 0, default, Main.rand.NextFloat(1f, 2.3f));
                            dust.scale = 1.5f;
                            dust.alpha = 100;
                            dust.noGravity = true;
                        }
                        if (ShootTimer >= 160)
                        {
                            for (int i = 0; i <= 5; i++)
                            {
                                Dust dust = Dust.NewDustPerfect(tipPosition - Projectile.velocity * dustSpot, DustID.Flare, shootVelocity.RotatedByRandom(MathHelper.ToRadians(18f)) * Main.rand.NextFloat(0.2f, 1.2f), 0, default, Main.rand.NextFloat(1f, 2.3f));
                                dust.scale = 2f;
                                dust.alpha = 100;
                                dust.noGravity = true;
                            }
                ...
                if (player.HasAmmo(heldItem) && !player.noItems && !player.CCed)
                {
                    ...
                    for (int j = 0; j < projectileCount; j++)
                    {
                        ...
                        if (ammoConsumed)
                        {
                            ...
                            Projectile.NewProjectile(source, spawnLocation, Vector2.Normalize(Projectile.velocity) * (speed * 2), ModContent.ProjectileType<ArchbeastArrow>(), damage * 15, knockBack * 5, Projectile.owner);
                        }
                    }
                    Projectile.Kill();
                }
